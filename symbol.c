...
int varint16_encode(ymd_int_t d, ushort_t rv[]) {
	ymd_uint_t coded = zigzag_encode(d);
	ymd_uint_t partal = 0;
	int i = 0;
	...
	goto bits_00;
bits_48:
	rv[i++] = (ushort_t)(partal >> 48);
	partal = coded & 0x0000ffff00000000ULL;
bits_32:
	rv[i++] = (ushort_t)(partal >> 32);
	partal = coded & 0x00000000ffff0000ULL;
bits_16:
	rv[i++] = (ushort_t)(partal >> 16);
	partal = coded & 0x000000000000ffffULL;
bits_00:
	rv[i++] = (ushort_t)partal;
	return i;
}
...
bits_48:
	raw |= ((ymd_uint_t)by[i++] << 48);
bits_32:
	raw |= ((ymd_uint_t)by[i++] << 32);
bits_16:
	raw |= ((ymd_uint_t)by[i++] << 16);
bits_00:
	raw |= by[i];
